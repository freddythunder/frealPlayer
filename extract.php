...
// if ($_SERVER['REQUEST_METHOD'] == "POST") {
	if ($_REQUEST['cmd'] ?? null) {
		...
		if ($cmd == 'getPlaylist' || $cmd == 'getSong') {
			...
			$playlist = ($vars['list'] ?? false);
			$song = ($vars['v'] ?? false);
			if ($playlist && !preg_match('/^[a-zA-Z0-9_-]+$/', (string)$playlist)) {
				$playlist = false;
			}
			if ($song && !preg_match('/^[a-zA-Z0-9_-]+$/', (string)$song)) {
				$song = false;
			}
			if ($cmd == 'getPlaylist') {
				$target = $playlist
			...
			error_log('background process run? ' . ($started ? 'yes' : 'no'));
...
<html>
<head>
<title>fRealPlayer :: Extract</title>
<meta charset="UTF-8">
...
<script>
if (localStorage.getItem('frealPlayerDarkMode') === '1') {
	document.documentElement.classList.add('dark-mode');
}
</script>
</head>
<body class="extractPage">
<script>
if (localStorage.getItem('frealPlayerDarkMode') === '1') {
...
<div id="wrapper">
	<header>
		<div class="extractTopBar">
			<a href="index.php" class="extractBack"><i class="fa fa-chevron-left"></i> Player</a>
		</div>
		<div id="songplaying">Extract from YouTube</div>
	</header>
	<div class="extractForm">
		...
		<button type="button" class="btn btn-primary w-100 mt-2" id="startButton">Start</button>
		<hr>
...
$(document).on('click', '#getSong', function(e) {
	if (!$('#albumpath').val()) {
		alert('no album path');
		return;
	}
